...
<<,fig.keep='high',fig.height=2.5, fig.width=4>>=
set.seed(05411)
compliance_data <- tibble(Income=rlnorm(1000,log(20000),log(100)),
                                        treat=rbinom(1000,1,0.5),
                          rnd=rnorm(1000,0,1)) %>%
  mutate(Complier=factor(case_when(treat==1 & Income>15000 & rnd>-0.2~0,
                            TRUE~1)),
         treat=case_when(treat==1~"Treated",
                         TRUE~"Control"))

compliance_data %>% ggplot() +
  geom_histogram(aes(x=Income, group=Complier, fill=Complier)) +
  facet_grid(~treat) +
  xlim(0,40000) +
  ylim(0,50) +
  theme_minimal() +
