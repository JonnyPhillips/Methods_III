...
The treatment is the ethnicity of the two recipients in the version of the dictator game that is presented to the subject. For the specific test of whether subjects treat co-ethnics differently, treatment is the presence of a recipient of the same ethnicity among the two recipients (while control is the absence of a co-ethnic). 
...
The average amount subjects transferred to co-ethnics is `r round(co_ethnic_amount,3)`. Since this is greater than an equal distribution of 5, this implies that there is a bias towards co-ethnics. 
...
The amount transferred to co-ethnics is greatest among the ethnicity `r Most_coethnic_bias`.

The most 'asymmetric' relationship is between `r most_asymm`. Bosniak subjects give much more to Croat recipients than Croat subjects give to Bosniak recipients.
...
The regression shows that, relative to the base category of a Bosniak recipient, there is no statistically significant difference in the amount Croats or Serbs receive on average.
...
In every game, however the subject chooses to allocate (5-5, 0-10) the average is always 5 (hence the intercept is exactly 5). So there is no difference at all between subject ethnicities and the regression can't differentiate the groups at all so the coefficient estimates are essentially zero. 
...
The base category is for a Bosniak subject and Bosniak recipient. They allocate on average 5.71. Bosniaks offer Croats 0.79 less (4.92), and Serbs 1.34 less (4.37). 

Croats offer Bosniaks 4.45 (5.71-1.26), Serbs 4.61 (5.71-1.26-1.34+1.50) and co-ethnic Croats 5.92 (5.71-1.26-0.79+2.26).

Serbs offer Bosniaks 4.6 (5.71-1.11), Croats 4.73 (5.71-1.11-0.79+0.92), and co-ethnic Serbs 5.62 (5.71-1.11-1.34+2.36).
...
The table is equivalent to the results in Table 3 of Whitt (2014). For example, the intercept of 5.55 corresponds to the first entry in their Table (Bosniak subject, Bosniak recipient, and Bosniak-Croat pairing).
...
The t-test we calculate here is smaller than that reported in Whitt (2014), 2.79 compared to 3.86.
...
The t statistic is now smaller, reflecting the increased uncertainty we have about whether the allocations are truly different once we take into account the dependencies in the data. Technically, the p-value is now greater than 0.05, so it is not clear that Bosniaks are differentiating between Croat and Serb respondents.
...
The results are very robust and do not change substantively with clustered standard errors.
...
The results are very similar, but not identical, to those in Table 6 of Whitt (2014). They show that making an equal allocation in the game is correlated with the survey response indicating they would treat people from other ethnic groups fairly. This suggests the results may be at least partially generalizable to real-world interactions.