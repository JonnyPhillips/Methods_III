...
\begin{frame}
\frametitle{Causal Inference}
<<OVB1,echo=FALSE,warning=FALSE,message=FALSE,fig.keep='high',fig.height=1.5, fig.width=4>>=
N <- 10000
...
<<OVB2,echo=FALSE,warning=FALSE,message=FALSE,fig.keep='high',fig.height=1.5, fig.width=4>>=
...
densities_y0 %>% ggplot() +
  ...
  ylab("density") +
  xlab("y0") +
  annotate("text", x=7,y=0.14,label="E(GDP % with NO \nDemocracy|NO Democracy)", size=2.5) +
  annotate("text", x=8,y=0.03,label="Africa",colour="#d95f0e")
@
...
densities_y1 %>% ggplot() +
  ...
  ylab("density") +
  xlab("y1") +
  annotate("text", x=16,y=0.14,label="E(GDP % with \nDemocracy|Democracy)",size=2.5) +
