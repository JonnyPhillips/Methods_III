...
10. An easier way to make a nice regression discontinuity plot is to use the _rdplot_ command in the _rdrobust_ package. Create this plot and be sure to specify the appropriate cutoff value based on the treatment assignment mechanism described above.
...
12. One way of easily picking an 'optimal' bandwidth (instead of assuming the range 480 to 520) is to use the automatic process in the _rdrobust_ command of the _rdrobust_ package. This method also provides more accurate standard errors. Apply the method, defining the cutoff appropriately and a quadratic running variable, and interpret the results.
