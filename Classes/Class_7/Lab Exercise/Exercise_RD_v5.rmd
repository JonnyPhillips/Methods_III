...
6. Now apply the 'full bandwidth' regression discontinuity method. This just means adding a control for the running variable, which in this case is income. Interpret the results. How do they compare to the treatment effect we specified?

```{r}
d %>% lm(y_obs ~ D + income, data=.) %>% summary()
```

7. Let's try and make a regression discontinuity plot manually to understand what's going on better. First, plot all the data, with the running variable on the x-axis and the observable outcome variable on the y-axis. What can you see in the graph?
...
8. The graph in Q7 is difficult to interpret so most regression discontinuity plots 'bin' the data into groups to more easily see the pattern. Bin the income data into 50 groups (try `cut()` in R), then calculate the average observed outcome in each bin and plot the two against each other.
...
9. An easier way to make a nice regression discontinuity plot is to use the _rdplot_ command in the _rdrobust_ package. Create this plot and be sure to specify the appropriate cutoff value based on the treatment assignment mechanism described above.
