...
The results fail to reject the null hypothesis of no sorting. There are a very similar number of units just either side of the threshold, and the distribution is simmilar to what we would expect if there was no sorting.
...
```{r, results='asis'}
d_PMDB %>% lm(Pct_Votes_2004~Incumbent + Win_Margin, data=.) %>% 
...
```{r, results='asis'}
d_PMDB %>% filter(Win_Margin<0.03 & Win_Margin>-0.03) %>% 
  lm(Pct_Votes_2004~Incumbent + Win_Margin, data=.) %>% 
...
rdrobust(d_PFL$Pct_Votes_2004, d_PFL$Win_Margin) %>% summary()

d_PSDB <- d %>% filter(SIGLA_PARTIDO=="PSDB")
rdrobust(d_PSDB$Pct_Votes_2004, d_PSDB$Win_Margin) %>% summary()
```
