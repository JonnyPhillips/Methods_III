...
```{r}
d <- d %>% mutate(D=case_when(2*gender+age/8+income/50+education*2>19~1,
                              T~0))
...
control_units <- d %>% filter(D==0 & gender==0 & education==1)
...
```{r}
d_temp <- d %>% mutate(gender=factor(gender),
                       education=factor(gender))
matched_data_Q8 <- matchit(D ~ gender + education + age + income, data=d_temp)
matched_data_Q8 %>% match.data()

#table(d$D,d$education)
#d %>% group_by(D) %>%
#  summarize(age=mean(income))
...
```{r}
matched_data_Q12 <- matchit(D ~ gender + education + age + income, data=data.frame(d), caliper=1.5)
