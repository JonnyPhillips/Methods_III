...
3. Now let's simulate potential outcomes - let's say the outcome is 'voter turnout' - for each municipality. We will assume that in general voter turnout is declining over time, and that the presence of oil reserves also reduces turnout. The treatment effect we will define as a 5\% extra voter turnout. 
...
4. Treatment $D$ is that the municipality is enrolled in a government program that they receive between time periods $t=1$ and $t=2$. Only some municipalities receive this in time $t=2$. We will assume that only municipalities with oil receive treatment. Make an indicator variable where each municipality with oil is coded as in the treated group and the rest as control. (*Note* that we are not coding municipalities as treated only in $t=2$, we are coding for whether they are a treated 'unit' which applies even in $t=0$, $t=1$).
...
6. First, let's run the 'naive' cross-sectional observational regression of observed outcomes on treatment. How does the result compare to our simulation assumptions? Why?
