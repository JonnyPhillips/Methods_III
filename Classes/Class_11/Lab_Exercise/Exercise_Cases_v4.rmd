...
$$y_0= \begin{cases}
1\text{ if } 2*inequality - \frac{growth}{3} + Europe>0\\ 
0 \text{ else}
\end{cases}$$ 
...
d <- d %>% mutate(Europe=ifelse(country=="Europe",1,0),
  y_0=case_when(2*inequality - growth/3 + Europe >0~1,
                                T~0),
...
(d) Can we conclude anything about the role of income level (treatment) on democrcay?

```{r}
d %>% filter(D==0 & y_obs==0) %>% sample_n(1)
```

8. Identify the control unit that has values of the confounders that ensure you can answer "No" to questions (a)-(c) in Question 7. It may not be possible - if not, try picking a different treated unit and then search for an appropriate control unit.
