...
10. That was the easy part. Now let's make our example more complicated. First, imagine that attitudes to redistribution are **NOT** our outcome variable of interest. Instead, our outcome of interest is how much people gave to charity. Generate a new set of potential outcomes for the variable for charity which depend on treatment:

$$Charity_{y0}=N(0,5) + \frac{age}{3} + 10*gender$$
$$Charity_y1=Charity_{y0} + 3$$
Then calculate observed outcomes for the Charity variable using the potential outcomes and treatment.

```{r}
d <- d %>% mutate(Charity_y0=rnorm(N,0,5) + age/3 + 10*gender + ,
                  Charity_y1=Charity_y0+3,
                  Charity_y_obs=case_when(D==1~Charity_y1,
                                          D==0~Charity_y0))
```

11. Now run the regression of observed charity outcomes on treatment, age, gender and attitude to redistribution ($y_{obs}$). 

```{r}
d %>% lm(Charity_y_obs~D + gender + age + y_obs, data=.) %>% summary()
```

11. Now run the regression of protest on treatment, age and gender. 

```{r}
d %>% lm(Charity_y_obs~D + gender + age, data=.) %>% summary()
```

, create two more variables, education and voting for the current government.
