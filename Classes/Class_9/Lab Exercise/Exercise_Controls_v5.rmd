...
$$income = N(500,50) + 100*gender + 10*Education$$
$$y_0=N(10,2)+\frac{age}{5} - 12*gender + 2*Education$$ 
$$y_1=y_0+5$$

```{r}
d <- d %>% mutate(education=rnorm(N,10,2),
                  income=rnorm(N,500,20) + 100*gender + 30*education,
                  y_0=rnorm(N,10,2) + age/5 - 12*gender + 2*education,
                  y_1=y_0+5,
                  y_obs=case_when(D==0~y_0,
                                  D==1~y_1))

summary(2*d$age+d$education+rnorm(3,2))

d <- d %>% mutate(education=rnorm(N,10,1),
                  religion=case_when(2*age+2*education+rnorm(3,1)>100~1,
                                     T~0),
...
d %>% ggplot() + geom_point(aes(education,age, colour=factor(religion)))
cor(d$education, d$age)
cor(d$education[d$religion==0], d$age[d$religion==0])
cor(d$education[d$religion==1], d$age[d$religion==1])

```

15. Draw the causal diagram (DAG) including all of these variables. Which variables do you need to add as control variables to recover the correct treatment effect?

16. If we just received a dataset with out treatment, outcome, gender, age and protest variables, we might consider these last three all potential confounder variables. Run the regression of observed attitude outcomes on treatment, gender, age and protest. Is the etimated treatment effect correct? Why?

```{r}
d %>% lm(y_obs~D + gender + age, data=.) %>% summary()
```

17. Now remove protest from your regression. Is the estimated treatment effect correct?
