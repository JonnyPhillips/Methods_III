...
14. Finally, create one more variable, Protest, so that people who have received the government program and have strong attitudes about redistribution are more likely to protest:

$$protest=\cases{1 \text{ if } \frac{1}{2}*D + y_{obs} > 120 \\ 0 \text{ else }}$$

```{r}
d <- d %>% mutate(protest=case_when(10*D + y_obs > 20~1,
                                    T~0))
```
...
```{r}
d %>% lm(y_obs~D + gender + age + protest, data=.) %>% summary()
```

17. Now remove protest from your regression. Is the etimated treatment effect correct?
