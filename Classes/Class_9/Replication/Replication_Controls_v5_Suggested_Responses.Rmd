...
stargazer(Q2, header=FALSE, title="Q2")
```
...
dag <- dagitty('dag{
               Connections_MP_Chief [pos="0,1"]
               temp_class_08 [pos="2,1"]
               Years_as_MP [pos="0,2"]
               Population [pos="0,0"]
               University [pos="-1,2"]
               Neighbourhood [pos="-1,1"]
               Job [pos="-1,0"]
               Random_meetings [pos="-1,-1"]
               
               Connections_MP_Chief -> temp_class_08
               Years_as_MP -> Connections_MP_Chief
               Years_as_MP -> temp_class_08
               Population -> Connections_MP_Chief
               Population -> temp_class_08
               University -> Connections_MP_Chief
               University -> temp_class_08
               Neighbourhood -> Connections_MP_Chief
               Job -> Connections_MP_Chief
               Random_meetings -> Connections_MP_Chief
}')
...
Q8 <- lm(tempclass08 ~ connectionsMPchief + univMP + 
           experienceMP + pop2000, data=d)
stargazer(Q8, header=FALSE, title="Q8")
``` 
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
Q9 <- zelig(tempclass08 ~ connectionsMPchief + univMP + 
           experienceMP + pop2000,data=d, model="ologit", cite=F) %>% from_zelig_model()
stargazer(Q9, header=FALSE, title="Q9")
```

A 1% increase in the number of years the chief has known the MP is associated with a `r round(100*(exp(log(101/100)*0.443)-1),3)`% ($100*(exp(log(101/100)*0.443)-1)$)  increase in the odds of having one more temporary classroom.
...
Q10 %>% stargazer(header=FALSE, title="Q10")
```

The coefficient of 0.352 is smaller than in the model in Q9. A 1% increase in the number of years the chief has known the MP is associated with a `r round(100*(exp(log(101/100)*0.352)-1),3)`% ($100*(exp(log(101/100)*0.352)-1)$)%  increase in the odds of having one more temporary classroom.
...
stargazer(list(Q10,Q11b,Q11c), header=FALSE, title="Q11")
```
