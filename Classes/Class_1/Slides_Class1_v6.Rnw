...
#Real coef of 1
data_orig <- tibble(x=rnorm(20,2,0.6),
               y_rnd=rnorm(20,0,0.6),
               y=x+y_rnd)

coefs <- seq(0,2,0.5)

lm((y_rnd+1*x)~x, data=data_orig) %>% 
  tidy() %>%
...
tilt <- function(coef,d){
  d %>% mutate(coef=coef,
               y_alt=y_rnd+coef*x,
               yhat=lm(y_alt~x)$fitted,
               intercept=lm(y_alt~x) %>% 
               tidy() %>%
               filter(term=="(Intercept)") %>%
               pull(estimate)) %>%
...
<<>>=
ols %>% ggplot() +
  geom_point(aes(x=coefficient,y=ssr))
@
\end{multicols}
\end{frame}
