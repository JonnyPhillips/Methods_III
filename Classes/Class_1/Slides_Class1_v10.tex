...
\frametitle{Predictions from Regressions}
\begin{itemize}
\item $$Attitude_i = \alpha + \beta_1 Income_i + \epsilon_i$$
\pause
\item $$Attitude_i = 2.235 + -0.000818 Income_i + N(0,2.378)$$
\pause
\item $Income_i=3000$ -> $Attitude_i = -0.219 + N(0,2.378)$
\pause
\item $Income_i=6000$ -> $Attitude_i = -2.673 + N(0,2.378)$
\pause
\item Increasing Income from 3000 to 6000 -> $\Delta Attitude_i = -0.219 - -2.673$
\end{itemize}
\end{frame}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in FUN(X[[i]], ...): object 'rich' not found}}\end{kframe}
