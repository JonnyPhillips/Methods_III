...
5. Next, let’s assume a specific Treatment Assignment Mechanism – random assignment. Unlike last week, treatment assignment will *not* depends on gender ($x$) or potential outcomes. Create a treatment variable D that gives each unit a 50\% chance of binary treatment (like a coin flip). *Hint: In R, try $rbinom$ and in Stata, try $rbinomial$.*
...
d. Now run the regression of observable outcomes ($y_{obs}$) on clustered treatment ($D\_cluster$). What is the confidence interval around the size of the effect $D\_cluster$? Is this accurate?
...
e. Now run the same regression again, but this time cluster the standard errors by the $cluster$ variable. How does the confidence interval on $D\_cluster$ differ from the previous analysis without clustered standard errors? *Try lm_robust in the estimatr package for clustered standard errors* 
...
14. (Advanced) Repeat the experiment and regression from Question 11 100 times with the same $x$, $y_0$ and $y_1$ every time, but allow treatment assignment $D$ to be re-randomized each time. For each experiment, perform the basic regression and calculate the 95\% confidence interval on the treatment variable. Finally, calculate how many of the 100 confidence intervals contain (cover) the real treatment effect ($c=2$).
